...
void dcm2euler(double *dc, double *result){
	...
	// save the result for FC and algorithm
	*(result + 0) = roll;
	*(result + 1) = pitch;
	*(result + 2) = yaw;
...
// for summation of sensor data, data type: a_x, a_y, a_z, w_x, w_y, w_z
static float SixDOF_data[6];
// function for FC
int GNC_coarse_alignment_Update(KVH_Readings_t KVHdata){
	for(int i = 0;i < 6;++i){
		SixDOF_data[i] += KVHdata[i];
	}
	return 0;
}
...
void GNC_coarse_alignment_Finish(double *result, double *SixDOF_data){
	...
	// mean of SixDOF data; sensor data without fusion
	double acc_x_m = (SixDOF_data[0] / size / delta_t);
	double acc_y_m = (SixDOF_data[1] / size / delta_t);
	double acc_z_m = (SixDOF_data[2] / size / delta_t);
	double gyro_x_m = (SixDOF_data[3] / size / delta_t) *deg2rad;
	double gyro_y_m = (SixDOF_data[4] / size / delta_t) *deg2rad;
	double gyro_z_m = (SixDOF_data[5] / size / delta_t) *deg2rad;
	// with fusion
